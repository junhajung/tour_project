...
<body>
   ...
   <main>
      <div class="container section-padding30">
         ...
         <div class="row">
            <form action="${pageContext.request.contextPath}/reply/batchdelete" method="post" id="form" style="width: -webkit-fill-available;">
               <table class="table">
                  <thead>
                     <tr>
                        <th scope="col">
                           <input type="button" value="[수정] " id="btn" style="background-color: transparent; color: blue; border: 0px transparent solid; cursor: pointer;">
                           <input type="submit" value="[삭제]" style="background-color: transparent; color: red; border: 0px transparent solid; cursor: pointer;">
                        </th>
                        ...
                        <tr>
                           ...
                           <th scope="row">
                              <div class="form-check">
                                 <input class="form-check-input" type="checkbox" name="chk[]" value="${list.id}" />
                              </div>
                           </th>
                           
                           <td><input type="text" name="userid[]" value="${list.userid}" readonly></td>
                           <td><input type="text" name="name[]" value="${list.name}" readonly></td>
                           <td><input type="text" name="reply[]" value="${list.reply}" ></td>
                           <td>
                           <fmt:formatDate value="${list.createdDate}" pattern="yyyy/MM/dd HH:mm"/>
                           <input type="hidden" name="createdDate[]" readonly>
                           </td>
...
$("#pagination").twbsPagination({
   ...
   onPageClick : function(event, page) {
      window.location.href = "${pageContext.request.contextPath}/user/myreply?page="+page;
   }
});
$(function(){
      // 태그 중 id가 btn인 것이 클릭되면
      $('#btn').click(function(){
         // 태그 중에서 id가 form인 것의 속성 action을 변경
         $('#form').attr('action', '${pageContext.request.contextPath}/reply/batchupdate');
         // 태그 중에서 id가 form인 것을 전송
         $('#form').submit();
      });
   });
</script>
